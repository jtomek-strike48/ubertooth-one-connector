...
use ubertooth_usb::protocol::{BlePacket, UsbPacket};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while start.elapsed() < Duration::from_secs(10) {
        ...
        if let Ok(64) = device.control_transfer_in(CMD_POLL, 0, 0, &mut buffer, 100) {
            total_packets += 1;
                                ...
                                Ok(ad_data) => {
                                    ...
                                    if devices_seen.insert(addr_str.clone()) {
                                        ...
                                        if !ad_data.service_uuids_128.is_empty() {
                                            println!(
                                                "   Services (128-bit): {} UUIDs",
                                                ad_data.service_uuids_128.len()
                                            );
                                        }
    ...
    if devices_seen.is_empty() {
        ...
        println!(
            "✅ Successfully parsed {} unique BLE devices",
            devices_seen.len()
        );
    }
